  ...
  if (!bypassAuth) {
    ...
    if (token) {
      try {
        ...
        // Verify (not just decode) the token so forged payloads cannot mint
        // fresh rate-limit buckets. Invalid tokens fall back to IP keying.
...
export const rateLimiters = {
  ...
  register: (req) => rateLimit(req, { limit: 15, windowMs: 60 * 60 * 1000, keyPrefix: "register", bypassAuth: true }),
  
  // General API: 100 requests per minute per user
  api: (req) => rateLimit(req, { limit: 100, windowMs: 60 * 1000, keyPrefix: "api" }),
  
  // Sensitive operations 30 per minute per user
  ...
  // AI generation (Gemini): 10 per minute per user
  ai: (req) => rateLimit(req, { limit: 10, windowMs: 60 * 1000, keyPrefix: "ai" }),
};